...
// Builds the primary sidebar option on the wp admin page and establishes what it will look like on front end. (Displayed by code in template file sidebar.php)
  function practice-theme-1_sidebar_widgets(){
    register_sidebar(array(
      'id' => 'primary-sidebar',
      'name' => esc_html__('Primary Sidebar', 'practice-theme-1'),
      'description' => esc_html__('This sidebar appears on the blog posts page', 'practice-theme-1'),
  ...
  $footer_layout = sanitize_text_field(get_theme_mod('practice-theme-1_footer_layout', '3,3,3,3'));
  $footer_layout = preg_replace('/\s+/', '', $footer_layout);
  // Explode takes as args: separator and string
  $columns = explode(',', $footer_layout);
  $footer_bg = practice-theme-1_sanitize_footer_bg(get_theme_mod( 'practice-theme-1_footer_bg', 'dark' ));
  $widget_theme = '';
  if($footer_bg == 'light'){
  ...
  foreach($columns as $i => $column){
    register_sidebar(array(
      ...
      'name' => sprintf(esc_html__('Footer Widget Column %s', 'practice-theme-1'), $i + 1),
      'description' => esc_html__('Footer Widgets', 'practice-theme-1'),
  ...
  add_action('widgets_init', 'practice-theme-1_sidebar_widgets');
?>
