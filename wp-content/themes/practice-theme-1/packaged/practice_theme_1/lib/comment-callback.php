<?php function practice_theme_1_comment_callback($comment, $args, $depth) {
  ...
  <article id="div-comment-<?php comment_ID();?>" class="c-comment__body">
    ...
    <?php edit_comment_link( esc_html__('Edit Comment', 'practice_theme_1'), '<span class="c-comment__edit-link">', '</span>' );?>
    <div class="c-comment__content">
      ...
      <a class="c-comment__time" href="<?php echo esc_url( get_comment_link($comment, $args))?>">
        <time datetime="<?php comment_time('c')?>">
          <?php
            printf(esc_html__('%s ago', 'practice_theme_1'),
      ...
      <?php if($comment->comment_approved == '0'){ ?>
        <p class="c-comment__awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting approval', 'practice_theme_1');?></p>
      <?php } ?>
      <?php
