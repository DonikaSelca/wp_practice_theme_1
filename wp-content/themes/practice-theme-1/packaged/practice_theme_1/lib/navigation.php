...
//  <!-- Register different menu names and locations  -->
  function practice_theme_1_registration_menus() {
  ...
  add_action('init', 'practice_theme_1_registration_menus', 'practice_theme_1_registration_menus');
  ...
  function practice_theme_1_aria_hasdropdown($atts, $item, $args){
  ...
  add_filter('nav_menu_link_attributes', 'practice_theme_1_aria_hasdropdown', 10, 3);
  ...
  function practice_theme_1_submenu_button($dir = 'down', $title){
    ...
    $button .= '<span class="u-screen-reader-text menu-button-show">' . sprintf(esc_html__('Show %s submenu', 'practice_theme_1'), $title) . '</span>';
    // By default menu is closed so we add 'aria-hiiden=true' so the first span of text shows. JS will toggle this class.
    $button .= '<span aria-hidden="true" class="u-screen-reader-text menu-button-hide">' . sprintf(esc_html__('Hide %s submenu', 'practice_theme_1'), $title) . '</span>';
  ...
  function practice_theme_1_dropdown_icon($title, $item, $args, $depth) {
    if($args->theme_location == 'main-menu') {
      ...
      if(in_array('menu-item-has-children', $item->classes)) {
        ...
        if($depth == 0) {
          $title .= practice_theme_1_submenu_button('down', $title);
        } else {
          $title .= practice_theme_1_submenu_button('right', $title);
        }
      }
    }
    return $title;
  }
add_filter( 'nav_menu_item_title', 'practice_theme_1_dropdown_icon', 10, 4 );
